...
# 2. Write a secret into kv:
    vault kv put user-kv/application01 password=secret username=Administrator
...
export SAM_CRED_LIB_ID=$(boundary credential-libraries create vault -credential-store-id ${CS_ID} -vault-path "user-kv/application01" -name "sam user" -format json | jq -r .item.id)
